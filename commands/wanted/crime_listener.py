...
def setup(bot):
    ...
    async def handle_crime_message(message: discord.Message):
        ...
        # UnbelievaBoat'un +crime embed'inden suç açıklamasını (varsa) çıkarıyoruz,
        # Wanted afişinde ve /criminal-record kaydında göstermek için.
        crime_description = message.embeds[0].description or message.embeds[0].title or "Bilinmeyen suç"
        ...
        if user.id in state.wanted_data:
            ...
            state.wanted_data[user.id]["crime"] = crime_description
        ...
        state.wanted_data[user.id] = {"bounty": bounty, "since": datetime.datetime.now(), "crime": crime_description}
        ...
        if announce_channel:
            embed = discord.Embed(
                title="🚨 WANTED",
                description=(
                    f"{user.mention} — Bounty: **{bounty} diamonds**\n"
                    f"Suç: *{crime_description}*"
                ),
